...
* **用户认证管理**: 用户注册、登录
* **知识库管理**: 支持多知识库分类管理，便于系统化组织和归档笔记内容
* **分组管理：**支持在知识库内以树形目录结构建立多级分组，可将笔记归入任意层级的分组，帮助系统化组织和归档笔记内容
* **笔记管理与编写**: 支持Markdown格式的笔记创建、编辑、删除等基本操作
* **批量导入功能**: 支持批量导入Markdown文件，快速构建知识库
* **智能AI问答：**基于指定知识库的RAG问答，支持多轮对话上下文管理，检索前自动结合上下文重写问题，提升多轮问答的检索准确率
* **MCP接口**: 提供标准的MCP接口，支持与其他AI工具集成
...
* 然后将每一个块生成向量存放到向量数据库 Qdrant 中
    ...
    > 支持指定知识库，全量/增量更新知识库的文档索引，并且使用SSE流式传输实时传输更新完成情况

#### 【2】AI问答

* 携带对话历史：如果存在历史对话，前端会携带最近几轮对话历史请求后端
* 问题重写：如果存在历史对话，后端会先尝试把当前问题改写成可独立检索的问题；如果本身已经完整，就直接使用原问题。
* 问题向量化：将用于检索的问题（原问题或重写后的问题）转化为向量
* 向量检索：在用户选定的知识库范围内，从向量数据库中检索与问题最相似的文档块
* 答案生成：将原始问题、最近几轮对话历史，以及检索到的文档块一起输入给 AI 模型生成答案
* 答案返回：SSE流式传输返回模型答案并附带命中的参考文档片段
...
* `/api/auth/*` - 认证相关接口
* `/api/knowledge-bases/*` - 知识库管理接口
* `/api/notes/*` - 笔记管理接口
* `/api/note-groups/*` - 分组管理接口
* `/api/rag/*` - RAG问答与索引接口
