...
飞猪在线笔记系统是一个基于 RAG（检索增强生成）和 MCP（Model Context Protocol）技术的智能笔记应用，支持本地和在线部署。系统采用现代化的前后端分离架构，结合人工智能技术，为用户提供高效、智能的笔记管理和知识检索体验。
...
MCP工具包括：
* `rag_query` - 基于知识库进行RAG问答查询

## 功能特性

### 批量导入

系统支持批量导入Markdown文件到指定知识库，提高知识库构建效率。导入过程会自动：
* 解析Markdown文件内容
* 按规则分块处理
* 生成向量索引
* 统计导入结果
